...
#
function tc_dopisy($texty,$fname,$pro='rozesilani',$vyrizuje,&$listu,$druh='D',$dne=null,$max=1,$new=1,$save=1) { trace();
//                                                 debug($texty,'texty');
...
//                                                 debug($c,"c je string:".is_string($c->ln),(object)array('gettype'=>1));
    ...
    $vyrizuje_telefon= $vyrizuje=='_user' ? $USER->options->telefon : $dop_rozesilani['telefon'];
    $vyrizuje_dne= $dne ? $dne : (
      $vyrizuje=='_user' ? date("j. n. Y") : $dop_rozesilani['dne']);
    $c->obsah= strtr($c->obsah,array(
      '{vyrizuje}'=>$vyrizuje_name,'{telefon}'=>$vyrizuje_telefon,'{vyrizeno}'=>$vyrizuje_dne
    ));
